package mk.ukim.finki.onlinecoursesbackend.util;
...
public class LoadSolution {
    ...
    private static void commitSolution(String index) {
        try {
            ...
            System.err.println(new BufferedReader(new InputStreamReader(createBranchProcess.getInputStream())).lines()
            ...
            System.err.println(new BufferedReader(new InputStreamReader(addChangesProcess.getInputStream())).lines()
                .collect(Collectors.joining("\n")));
            ...
            System.err.println(new BufferedReader(new InputStreamReader(commitProcess.getInputStream())).lines()
                .collect(Collectors.joining("\n")));
            ...
            System.err.println(new BufferedReader(new InputStreamReader(pushProcess.getInputStream())).lines()
                .collect(Collectors.joining("\n")));
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static void getAndLoadLastSolution(String index) throws IOException {
        ...
        ResponseEntity<String> response =
            restTemplate.exchange("http://localhost:8080/api/grading/" + examId + "/last_submission/full/" + index,
                HttpMethod.GET, new HttpEntity<>(createHeaders(System.getenv("WP_USER"), System.getenv("WP_PASS"))),
                String.class);
    ...
    public static void getSolutionById(Integer id) throws IOException {
        RestTemplate restTemplate = new RestTemplate();
        ResponseEntity<String> response =
            restTemplate.exchange("http://localhost:8080/api/grading/submission/" + id, HttpMethod.GET,
                new HttpEntity<>(createHeaders(System.getenv("WP_USER"), System.getenv("WP_PASS"))), String.class);
        loadSolution(new ByteArrayInputStream(response.getBody().getBytes()));
    }
